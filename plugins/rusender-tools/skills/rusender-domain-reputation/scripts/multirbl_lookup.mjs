...
// plugins/rusender-domain-reputation/skills/rusender-domain-reputation/scripts/multirbl_lookup.ts
var import_node_html_parser = __toESM(require_dist(), 1);
var BASE_URL = "https://multirbl.valli.org";
var FORM_URL = `${BASE_URL}/index.php`;
var LOOKUP_URL = `${BASE_URL}/json-lookup.php`;
var REPORT_URL = `${BASE_URL}/dnsbl-lookup/`;
var USER_AGENT = "RuSender-domain-reputation/1.0 (+local best-effort check)";
...
async function postForm(url, data, timeoutSeconds) {
...
async function startReport(domain, timeoutSeconds) {
  ...
  const root = (0, import_node_html_parser.parse)(page);
  const rows = root.querySelectorAll("tr").filter((row) => (row.getAttribute("id") ?? "").startsWith("DNSBLBlacklistTest_")).flatMap((row) => {
...
async function checkRow(sessionHash, row, domain, timeoutSeconds) {
  try {
