...
/** Store one sanitized RuSender email-deliverability result as private JSON. */
...
function parseCli(args: string[]): {result: string; dataDirectory: string} {
  let result = '';
...
async function main(): Promise<void> {
  ...
  const value: unknown = JSON.parse(await readFile(options.result, 'utf8'));
